...
class EnhancedWebCrawler:
    ...
    def extract_categories(self):
        try:
            ...
            # Fixed lambda syntax: replaced any$$...$$ with any(...)
...
def main():
    st.set_page_config(page_title="Enhanced Web Crawler", page_icon="🕷️", layout="wide")
    
    st.markdown('<h1>Enhanced Web Crawler</h1>', unsafe_allow_html=True)
    st.markdown('<p>Search for GoWithGuide and affiliate references with advanced detection.</p>', unsafe_allow_html=True)
    ...
    with col1:
        start_btn = st.button("🕷️ Start Crawling", help="Begin crawling")
    with col2:
    ...
    if stop_btn:
        ...
        if st.session_state.crawler:
            ...
            status_placeholder.write(st.session_state.current_status)
        st.session_state.crawler = None
        # Do not clear displayed_results
    ...
    if st.session_state.running and st.session_state.crawler:
        ...
        if crawler.crawl_mode == "Quick":
            ...
            st.session_state.current_status = f"Crawling: {crawler.start_url}"
            status_placeholder.write(st.session_state.current_status)
            for i, url in enumerate(urls_to_crawl):
                if not st.session_state.running:
                    break
                crawler.process_url(url)
                new_results = [result for result in crawler.results if result not in st.session_state.displayed_results]
                for result in new_results:
                    st.markdown(format_result(result), unsafe_allow_html=True)
                    st.session_state.displayed_results.append(result)
            if st.session_state.running:
                if crawler.results:
                    st.session_state.current_status = f"Crawl finished. Matches found: {len(crawler.results)}"
                else:
                    st.session_state.current_status = "Crawl finished. No matches found"
            else:
                st.session_state.current_status = "Crawl stopped by user"
            status_placeholder.write(st.session_state.current_status)
            st.session_state.running = False
    ...
    if st.session_state.displayed_results:
        st.markdown('<h2>Matches Found</h2>', unsafe_allow_html=True)
